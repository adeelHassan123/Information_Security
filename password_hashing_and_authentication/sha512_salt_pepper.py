...
def sha512(message):
    ...
    for i in range(0, len(message), 128):
        ...
        a, b, c, d, e, f, g, h_ = h
        for t in range(80):
            t1 = (h_ + ((e & f) ^ (~e & g)) + K[t] + w[t]) & 0xFFFFFFFFFFFFFFFF
...
def register_user(username, password):
    ...
    hashed = sha512((salt + password + PEPPER).encode('utf-8'))
    database[username] = {'salt': salt, 'hash': hashed}
...
def verify_user(username, password):
    ...
    return sha512((salt + password + PEPPER).encode('utf-8')) == stored_hash
...
password_check = input("Re-enter password to verify: ")
if verify_user(username, password_check):
